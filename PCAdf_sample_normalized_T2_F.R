...
label.df_sample_normalized_T2_F = label.df_Tx

#boxplot
boxplot(scale(data.df_Tx, center = T, scale = F), col = 'gold', las = 2)

#perform the pca on the dataset
pc.df_sample_normalized_T2_F = princomp(df_sample_normalized_T2_F[,-(1:3)])
...
for(i in 1:3) barplot(load.df_sample_normalized_T2_F[,i], ylim = c(-1, 1), main=paste('Loadings PC ',i,sep=''))
...
abline(h=1, col='blue')
barplot(sapply(df_sample_normalized_T2_F,sd)^2, las=2, main='Original Variables', ylim=c(0,2.5e6), ylab='Variances')
...
axis(1,at=1:ncol(df_sample_normalized_T2_F),labels=1:ncol(df_sample_normalized_T2_F),las=2)
...
for(i in 1:n){
  ...
  col.speakers[i] = col.palette_speaker[which(label.df_sample_normalized_T2_F[i,3] == levels(label.df_sample_normalized_T2_F[,3]))]
}

#plot of the scores
x11()
plot(scores.df_sample_normalized_T2_F[,1:2], col = col.diphtongs, pch=19,
     main=paste('Scores per diphtong, with both speakers and formant T', formante))
